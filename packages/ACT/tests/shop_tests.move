...
module act::genesis_shop_tests {
    ...
    fun test_gang_mk_iv_chestpieces(world: &mut World, index: u64, rarity: vector<u8>, colour_way: vector<u8>) {
        assert_eq(world.genesis_shop.borrow_mut().borrow(attributes::chestpiece())[index].rarity(), rarity.to_string());
        assert_eq(world.genesis_shop.borrow_mut().borrow(attributes::chestpiece())[index].colour_way(), colour_way.to_string());
        assert_eq(world.genesis_shop.borrow_mut().borrow(attributes::chestpiece())[index].manufacturer(), b"ExoTech Solutions".to_string());
        assert_eq(world.genesis_shop.borrow_mut().borrow(attributes::chestpiece())[index].name(), b"Fang MK IV".to_string());
    }

    fun test_helios_chestpieces(world: &mut World, index: u64, rarity: vector<u8>, colour_way: vector<u8>) {
        assert_eq(world.genesis_shop.borrow_mut().borrow(attributes::chestpiece())[index].rarity(), rarity.to_string());
        assert_eq(world.genesis_shop.borrow_mut().borrow(attributes::chestpiece())[index].colour_way(), colour_way.to_string());
        assert_eq(world.genesis_shop.borrow_mut().borrow(attributes::chestpiece())[index].manufacturer(), b"Zenith Aerospace".to_string());
        assert_eq(world.genesis_shop.borrow_mut().borrow(attributes::chestpiece())[index].name(), b"Helios".to_string());
    }

    fun test_neo_shogunate_chestpieces(world: &mut World, index: u64, rarity: vector<u8>, colour_way: vector<u8>) {
        assert_eq(world.genesis_shop.borrow_mut().borrow(attributes::chestpiece())[index].rarity(), rarity.to_string());
        assert_eq(world.genesis_shop.borrow_mut().borrow(attributes::chestpiece())[index].colour_way(), colour_way.to_string());
        assert_eq(world.genesis_shop.borrow_mut().borrow(attributes::chestpiece())[index].manufacturer(), b"Obsidian Dynamics".to_string());
        assert_eq(world.genesis_shop.borrow_mut().borrow(attributes::chestpiece())[index].name(), b"Neo-Shogunate".to_string());
    }
