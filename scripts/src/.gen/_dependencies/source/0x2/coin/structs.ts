...
export function isCoin(type: string): boolean { type = compressSuiType(type); return type.startsWith(`${PKG_V20}::coin::Coin` + '<'); }
...
export function isCoinMetadata(type: string): boolean { type = compressSuiType(type); return type.startsWith(`${PKG_V20}::coin::CoinMetadata` + '<'); }
...
export class CoinMetadata<T extends PhantomTypeArgument> implements StructClass { static readonly $typeName = `${PKG_V20}::coin::CoinMetadata`; static readonly $numTypeParams = 1;

 readonly $typeName = CoinMetadata.$typeName;

 readonly $fullTypeName: `${typeof PKG_V20}::coin::CoinMetadata<${PhantomToTypeStr<T>}>`;
 ...
 private constructor(typeArgs: [PhantomToTypeStr<T>], fields: CoinMetadataFields<T>, ) { this.$fullTypeName = composeSuiType( CoinMetadata.$typeName, ...typeArgs ) as `${typeof PKG_V20}::coin::CoinMetadata<${PhantomToTypeStr<T>}>`; this.$typeArgs = typeArgs;
...
export function isCurrencyCreated(type: string): boolean { type = compressSuiType(type); return type.startsWith(`${PKG_V20}::coin::CurrencyCreated` + '<'); }
...
export function isDenyCap(type: string): boolean { type = compressSuiType(type); return type.startsWith(`${PKG_V20}::coin::DenyCap` + '<'); }
...
export function isRegulatedCoinMetadata(type: string): boolean { type = compressSuiType(type); return type.startsWith(`${PKG_V20}::coin::RegulatedCoinMetadata` + '<'); }
...
export function isTreasuryCap(type: string): boolean { type = compressSuiType(type); return type.startsWith(`${PKG_V20}::coin::TreasuryCap` + '<'); }
