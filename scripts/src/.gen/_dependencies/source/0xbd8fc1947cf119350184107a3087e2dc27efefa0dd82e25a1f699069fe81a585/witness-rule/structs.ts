...
export class Rule<Proof extends PhantomTypeArgument> implements StructClass { __StructClass = true as const;

 static readonly $typeName = `${PKG_V1}::witness_rule::Rule`; static readonly $numTypeParams = 1; static readonly $isPhantom = [true,] as const;

 readonly $typeName = Rule.$typeName; readonly $fullTypeName: `${typeof PKG_V1}::witness_rule::Rule<${PhantomToTypeStr<Proof>}>`; readonly $typeArgs: [PhantomToTypeStr<Proof>]; readonly $isPhantom = Rule.$isPhantom;
 ...
 static fromSuiObjectData<Proof extends PhantomReified<PhantomTypeArgument>>( typeArg: Proof, data: SuiObjectData ): Rule<ToPhantomTypeArgument<Proof>> { if (data.bcs) { if (data.bcs.dataType !== "moveObject" || !isRule(data.bcs.type)) { throw new Error(`object at is not a Rule object`); }

 const gotTypeArgs = parseTypeName(data.bcs.type).typeArgs; if (gotTypeArgs.length !== 1) { throw new Error(`type argument mismatch: expected 1 type argument but got '${gotTypeArgs.length}'`); }; const gotTypeArg = compressSuiType(gotTypeArgs[0]); const expectedTypeArg = compressSuiType(extractType(typeArg)); if (gotTypeArg !== compressSuiType(extractType(typeArg))) { throw new Error(`type argument mismatch: expected '${expectedTypeArg}' but got '${gotTypeArg}'`); };

 return Rule.fromBcs( typeArg, fromB64(data.bcs.bcsBytes) ); } if (data.content) { return Rule.fromSuiParsedData( typeArg, data.content ) } throw new Error( "Both `bcs` and `content` fields are missing from the data. Include `showBcs` or `showContent` in the request." ); }
