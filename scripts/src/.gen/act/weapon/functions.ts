...
export function new_( tx: Transaction, args: NewArgs ) { return tx.moveCall({ target: `${PUBLISHED_AT}::weapon::new`, arguments: [ pure(tx, args.name, `${String.$typeName}`), pure(tx, args.imageUrl, `${String.$typeName}`), pure(tx, args.imageHash, `${String.$typeName}`), pure(tx, args.modelUrl, `${String.$typeName}`), pure(tx, args.slot, `${String.$typeName}`), pure(tx, args.colourWay, `${String.$typeName}`), pure(tx, args.edition, `${String.$typeName}`), pure(tx, args.manufacturer, `${String.$typeName}`), pure(tx, args.rarity, `${String.$typeName}`), pure(tx, args.hash, `${String.$typeName}`), pure(tx, args.wearRating, `u64`) ], }) }
...
export function imageHash( tx: Transaction, self: TransactionObjectInput ) { return tx.moveCall({ target: `${PUBLISHED_AT}::weapon::image_hash`, arguments: [ obj(tx, self) ], }) }
