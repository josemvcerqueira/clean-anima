...
export interface InitStateArgs { otw: GenericArg; name: string | TransactionArgument; description: string | TransactionArgument }

export function initState( tx: Transaction, typeArgs: [string, string, string], args: InitStateArgs ) { return tx.moveCall({ target: `${PUBLISHED_AT}::item::init_state`, typeArguments: typeArgs, arguments: [ generic(tx, `${typeArgs[0]}`, args.otw), pure(tx, args.name, `${String.$typeName}`), pure(tx, args.description, `${String.$typeName}`) ], }) }
