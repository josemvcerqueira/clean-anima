...
(async () => {
	...
	execSync(
		`${process.env.CLI_PATH!} move build --dump-bytecode-as-base64 --path ${process.env.ANIMA_PACKAGE_PATH!}`, 
		{ encoding: 'utf-8' }
	));
	...
	try {		
		...
		fs.writeFileSync('./src/data/anima-created.json', JSON.stringify(objects, null, 2));
